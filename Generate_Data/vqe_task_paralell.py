...
class VqeTrainerNew:
    ...
    def batch_train_parallel(self, samples,device_name, task_name, run_id):
        ...
        return energy_f, param_f, energy_epoch_f, duration


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--noise", type=bool, default=True, help="Whether to consider noise")
    parser.add_argument("--two_qubit_depolarizing_p", type=float, default=0.01, help="Two-qubit depolarizing noise strength")
    parser.add_argument("--single_qubit_depolarizing_p", type=float, default=0.001, help="Single-qubit depolarizing noise strength")
    parser.add_argument("--bit_flip_p", type=float, default=0.01, help="Bit-flip noise strength")
    parser.add_argument("--run_id", type=int, default=0, help="Bit-flip noise strength")
    ...
    device_name = args.device_name   # see config.py
    task_name = args.task
    qubit = 8
    ...
    save_datasets = f'../datasets/experiment_data/{device_name}_{task_name}/'
    samples = utils.load_pkl(save_datasets + f'training/run_{args.run_id}/samples.pkl')
    trainer.batch_train_parallel(samples, device_name, task_name, args.run_id)
